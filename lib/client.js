...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    var state = {
      ...
      scanTimer: null, scanConfirm: null, scanPick: 'quick',
    ...
    function injectStyle() {
      ...
      s.textContent = [
        ...
        // 绿色实底"行动按钮"：与蓝描边的模式选择按钮明显区分（开始查杀专用）
        '.lg-btn.success{background:#16a34a;border-color:#16a34a;color:#fff;font-weight:600}',
        '.lg-btn.success:hover:not(:disabled){background:#15803d;border-color:#15803d;color:#fff}',
        '.lg-btn.success:disabled{background:#16a34a44;border-color:#16a34a44;color:#ffffffaa}',
    ...
    function renderScan() {
      var b = body();
      var running = !!(state.scan && state.scan.running);
      var pick = state.scanPick || 'quick';
      // 模式按钮只负责选中（不启动）；"开始查杀"按钮执行启动
      var quickCls = 'lg-btn' + (pick === 'quick' && !running ? ' primary' : '') + (running ? ' disabled' : '');
      var fullCls = 'lg-btn' + (pick === 'full' && !running ? ' primary' : '') + (running ? ' disabled' : '');
      b.innerHTML =
        '<div class="lg-card"><div class="lg-card-title">' + I('bug') + ' Windows Defender 查杀</div>' +
        '<div class="lg-muted" style="margin-bottom:12px">直接调用系统 Windows Defender 引擎执行真实扫描。先选择查杀模式，再点下方的绿色"开始查杀"。快速查杀数分钟，全面查杀可能需要数小时，期间可关闭本窗口，扫描继续进行。</div>' +
        '<div class="lg-row">' +
        '<button class="' + quickCls + '" id="lg-scan-quick">' + I('zap', 12) + ' 快速查杀</button>' +
        '<button class="' + fullCls + '" id="lg-scan-full">' + I('search', 12) + ' 全面查杀</button>' +
        '<button class="lg-btn sm" id="lg-scan-refresh" style="margin-left:auto"' + (running ? ' disabled' : '') + '>' + I('refresh', 12) + ' 刷新状态</button></div>' +
        '<div class="lg-row" style="margin-top:12px">' +
        '<button class="lg-btn success" id="lg-scan-start"' + (running ? ' disabled' : '') + ' style="min-width:180px;justify-content:center">' + I('play', 13) + ' 开始' + (pick === 'full' ? '全面' : '快速') + '查杀</button>' +
        '<span class="lg-muted">点此启动所选模式的查杀</span></div>' +
        (state.scanConfirm
      ...
      b.querySelector('#lg-scan-quick').addEventListener('click', function () {
        if (running) return;
        state.scanPick = 'quick'; state.scanConfirm = false;
        renderScan(); // 只切换选中态，不启动
      });
      b.querySelector('#lg-scan-full').addEventListener('click', function () {
        if (running) return;
        state.scanPick = 'full'; state.scanConfirm = false;
        renderScan(); // 只切换选中态，不启动
      });
      b.querySelector('#lg-scan-start').addEventListener('click', function () {
        if (running) return;
        // 全面查杀先弹确认条；快速查杀直接启动
        if (state.scanPick === 'full') {
          state.scanConfirm = true;
          renderScan();
        } else {
          startScan('quick');
        }
    ...
    function startScan(mode) {
      ...
      api('/scan/start', { method: 'POST', body: JSON.stringify({ mode: mode }) }).then(function (r) {
        ...
        if (state.tab === 'scan') renderScan(); // 整页重绘：确认条必然消失、按钮进入禁用态
      });
    }
    ...
    /** 侧栏按钮呼吸点（查杀或磁盘分析任一运行即点亮；由 syncBadge 统一计算） */
    function setScanBadge() { syncBadge(); }
    /** 查杀历史卡片（读取宿主持久化记录） */
    ...
    /** 同步模式按钮与开始按钮的选中/禁用态（扫描结束/状态刷新时无需整页重绘） */
    function syncScanButtons() {
      var b = modal && modal.querySelector('#lg-scan-quick');
      var f = modal && modal.querySelector('#lg-scan-full');
      var s = modal && modal.querySelector('#lg-scan-start');
      var r = modal && modal.querySelector('#lg-scan-refresh');
      if (!b || !f || !s) return;
      var running = !!(state.scan && state.scan.running);
      var pick = state.scanPick || 'quick';
      b.disabled = running;
      ...
      b.className = 'lg-btn' + (pick === 'quick' && !running ? ' primary' : '');
      f.className = 'lg-btn' + (pick === 'full' && !running ? ' primary' : '');
      s.innerHTML = I('play', 13) + ' 开始' + (pick === 'full' ? '全面' : '快速') + '查杀';
    }
    ...
    function renderNet() {
      ...
      b.innerHTML =
        ...
        '<div class="lg-muted" style="margin-bottom:10px">真实下载 + 真实上传测速，按下行带宽评级。</div>' +
    ...
    function rateSpeed(mbps) {
      ...
      if (mbps < 5) return { name: '婴儿车', tone: 'bad', desc: '够日常刷网页、聊微信' };
      if (mbps < 30) return { name: '汽车', tone: 'warn', desc: '高清视频、视频通话没问题' };
      if (mbps < 100) return { name: '火车', tone: 'ok', desc: '4K 流媒体、大型下载都从容' };
      if (mbps < 500) return { name: '飞机', tone: 'ok', desc: '几乎秒下，多设备并发无忧' };
      return { name: '火箭', tone: 'ok', desc: '顶级宽带，职业级速度' };
    }
    function runSpeedTest() {
      var st = modal.querySelector('#lg-net-state'); var res = modal.querySelector('#lg-net-result');
      if (st) st.textContent = '测下行带宽中…';
      if (res) res.innerHTML = '';
      api('/net/speed', { method: 'POST', body: '{}' }).then(function (r) {
        if (st) st.textContent = '';
        var html = (res && res.innerHTML || '');
        if (!r.ok) {
          state.netResult = html + '<div class="lg-err">' + esc(r.error) + '</div>';
          if (res) res.innerHTML = state.netResult;
          return;
        }
        var down = r.mbps, up = r.upMbps;
        var rating = rateSpeed(down);
        var sp =
          '<div class="lg-kv"><span>下行带宽</span><b style="color:var(--dsw-alias-state-business-primary,#5ad1ff);font-size:15px">' +
          (down == null ? '失败' : down + ' Mbps') + '</b></div>' +
          '<div class="lg-kv"><span>上行带宽</span><b style="color:#4ade80;font-size:15px">' +
          (up == null ? '失败' : up + ' Mbps') + '</b></div>';
        var ratingHtml = rating
          ? '<div style="margin-top:10px;padding:12px 14px;border-radius:10px;border:1px solid ' +
            (rating.tone === 'ok' ? 'rgba(74,222,128,.45)' : rating.tone === 'warn' ? 'rgba(251,191,36,.45)' : 'rgba(248,113,113,.45)') +
            ';background:' + (rating.tone === 'ok' ? 'rgba(74,222,128,.08)' : rating.tone === 'warn' ? 'rgba(251,191,36,.08)' : 'rgba(248,113,113,.08)') +
            '"><div style="font-size:14px;font-weight:600;color:' +
            (rating.tone === 'ok' ? '#4ade80' : rating.tone === 'warn' ? '#fbbf24' : '#f87171') +
            '">你的下行带宽 ≈ ' + rating.name + '</div>' +
            '<div class="lg-muted" style="margin-top:2px">' + rating.desc + '（按下行带宽评级）</div></div>'
          : '';
        state.netResult = html + sp + ratingHtml;
        if (res) res.innerHTML = state.netResult;
      });
    }
    ...
    function renderSys() {
      ...
      b.innerHTML =
        ...
        '</div>' +
        '<div class="lg-card"><div class="lg-card-title">' + I('trash') + ' 一键清理缓存</div>' +
        '<div class="lg-muted" style="margin-bottom:8px">清理系统临时文件与浏览器缓存（%TEMP%、Chrome/Edge/Firefox 缓存）。无需管理员，几秒完成；被占用的文件会自动跳过，绝不触碰用户文档。</div>' +
        '<div class="lg-row"><button class="lg-btn sm" id="lg-cache-clean">' + I('trash', 12) + ' 立即清理</button><span class="lg-muted" id="lg-cache-res" style="margin-left:8px"></span></div>' +
        '</div>' +
        '<div class="lg-card"><div class="lg-card-title">' + I('disk') + ' 磁盘</div><div id="lg-disks"></div></div>' +
      ...
      b.querySelector('#lg-cache-clean').addEventListener('click', cleanCache);
      // 先渲染上次快照（秒开），再拉最新数据覆盖
      if (state.sysCache) {
    ...
    /** 侧栏呼吸点：查杀进行中点亮 */
    function syncBadge() {
      var on = !!(state.scan && state.scan.running);
      if (!railBtn) return;
    ...
    function freeMem() {
      ...
      api('/mem/free', { method: 'POST', body: '{}' }).then(function (r) {
        ...
        if (!out) return;
        if (!r.ok) {
          // 提权路径：可能弹出了 UAC 让用户确认/拒绝
          out.textContent = r.error || '释放失败';
          if (state.admin === false && /UAC|授权/.test(r.error || '')) {
            out.textContent = r.error + '——点一下系统弹窗里的"是"就会真正释放';
          }
          return;
        }
        var res = r.result;
        var freed = Math.max(0, res.freedMB);
        var msg;
        if (r.elevated || state.admin) {
          // 提权/管理员路径：主菜是清空待机内存
          msg = '可用内存增加约 ' + freed + ' MB' +
            (res.standbyOK === 0 ? '（已清空系统待机内存）' : '（待机内存清空受限）') +
            (res.trimmed > 0 ? '，另收缩 ' + res.trimmed + ' 个进程工作集' : '') +
            (r.elevated ? '（已通过管理员授权执行）' : '');
        } else {
          msg = '已收缩 ' + res.trimmed + ' 个进程工作集（' + (res.skipped || 0) + ' 个被拒）' + (freed ? '，可用内存 +' + freed + ' MB' : '');
          msg += '。当前为普通权限，清空待机内存需要管理员——点一下系统弹窗里的「是」即可真正释放';
        }
        out.textContent = msg;
      });
    }
    /** 一键清理缓存：无需提权，秒级完成（临时文件 + 浏览器缓存） */
    function cleanCache() {
      var btn = modal.querySelector('#lg-cache-clean'); var out = modal.querySelector('#lg-cache-res');
      if (btn) { btn.disabled = true; btn.textContent = '清理中…'; }
      if (out) out.textContent = '';
      api('/cache/clean', { method: 'POST', body: '{}' }).then(function (r) {
        if (btn) { btn.disabled = false; btn.innerHTML = I('trash', 12) + ' 立即清理'; }
        if (!out) return;
        if (!r.ok) { out.textContent = r.error || '清理失败'; return; }
        var detail = (r.rows || []).filter(function (x) { return x.freedMB > 0; })
          .map(function (x) { return x.name + ' ' + x.freedMB + ' MB'; }).join('、');
        out.textContent = '已释放 ' + r.totalFreedMB + ' MB' + (detail ? '（' + detail + '）' : '（没有可清理的缓存）');
      });
    }
    ...
    function apply(ctx) {
      ...
      api('/scan/status').then(function (r) {
        if (r && r.running) { state.scan = r; startScanWatch(); setScanBadge(); }
      });
