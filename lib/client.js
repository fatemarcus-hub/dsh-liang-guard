  ...
  factory: function (require) {
    ...
    function applyScanStatus(r) {
      ...
      if (wasRunning && !r.running) {
        notifyScanDone(r);
        // 扫描刚结束：顺手刷新威胁记录，用户无需再手动点"刷新状态"
        if (modal && modal.style.display !== 'none' && state.tab === 'scan') refreshThreats(true);
      }
    ...
    function renderSys() {
      ...
      b.innerHTML =
        ...
        '<div class="lg-card"><div class="lg-card-title">' + I('memory') + ' 内存</div><div id="lg-mem"></div><canvas class="lg-spark" id="lg-mem-canvas" width="300" height="44"></canvas></div>' +
        '</div>' +
    ...
    function cleanCache() {
      ...
      api('/cache/clean', { method: 'POST', body: '{}' }).then(function (r) {
        ...
        var total = Math.max(0, r.totalFreedMB); // 清理期间若有程序在写临时文件，差值可能为负
        var detail = (r.rows || []).filter(function (x) { return x.freedMB > 0; })
          .map(function (x) { return x.name + ' ' + x.freedMB + ' MB'; }).join('、');
        out.textContent = '已释放 ' + total + ' MB' + (detail ? '（' + detail + '）' : '（没有可清理的缓存）');
      });
    }
