...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    /** 侧栏呼吸点：查杀 或 磁盘分析 任一在进行中就点亮（需求：分析也像查杀那样挂着） */
    function syncBadge() {
      var on = !!((state.scan && state.scan.running) || (state.diskState && state.diskState.running));
      if (!railBtn) return;
    ...
    function renderDiskResult() {
      ...
      try {
        ...
        if (d && d.running) {
          host.innerHTML = '<div class="lg-scan-run"><div class="lg-spin"></div><div style="flex:1"><div style="font-size:13px;color:var(--dsw-alias-label-primary,#e8eef7)">' + esc(d.drive) + ': 盘分析进行中 · 已用 ' + fmtMs(d.elapsedMs || 0) + '</div><div class="lg-scanbar" style="max-width:420px"></div>' +
            '<div class="lg-muted">正在遍历全盘统计目录体积（大磁盘可能需要数分钟到十几分钟）</div></div>' +
            '<button class="lg-btn sm danger" id="lg-disk-cancel">' + I('stop', 11) + ' 取消</button></div>';
          var c = host.querySelector('#lg-disk-cancel');
          if (c) c.addEventListener('click', function () {
            c.disabled = true;
            c.innerHTML = I('refresh', 11) + ' 取消中…';
            api('/disk/cancel', { method: 'POST', body: '{}' }).then(function (r) {
              if (r.ok) showToast('磁盘分析已取消', 'ok');
              refreshDisk();
              syncBadge();
            });
          });
        ...
        // 防呆：单条结果可能被序列化成对象而非数组（曾导致 map 抛错、整个系统页渲染中断）
        var dirs = Array.isArray(res.dirs) ? res.dirs : [];
        var files = Array.isArray(res.files) ? res.files : [];
        state.diskResults[res.drive] = res;
        var maxDir = dirs.length ? dirs[0].sizeGB : 1;
        var dirRows = dirs.map(function (x) {
        ...
        // 任何渲染异常都不能拖垮整个系统页（曾因此导致 TOP 进程表格消失）
        host.innerHTML = '<div class="lg-err">分析结果渲染失败：' + esc(String((e && e.message) || e)) + '</div>';
      }
    }
    ...
    function freeMem() {
      ...
      api('/mem/free', { method: 'POST', body: '{}' }).then(function (r) {
        ...
        var res = r.result;
        var msg = '已收缩 ' + res.trimmed + ' 个进程工作集' + (res.skipped ? '（' + res.skipped + ' 个被拒）' : '') + '，可用内存变化 ' + (res.freedMB >= 0 ? '+' : '') + Math.max(0, res.freedMB) + ' MB';
        // 非管理员时 Windows 禁止收缩他人进程（EmptyWorkingSet 实测全失败），如实说明
        if (!state.admin && res.trimmed === 0) {
          msg += '。当前为普通权限，Windows 不允许收缩其他进程内存——以管理员身份运行 DSH 后此功能才有效';
        }
        out.textContent = msg;
      });
    }
    ...
    function apply(ctx) {
      ...
      // 页面刷新后恢复磁盘分析守望（与查杀同理：分析还在跑就重新挂呼吸灯）
      api('/disk/status').then(function (r) {
        if (r && r.ok && r.running) { state.diskState = r; startDiskWatch(); setScanBadge(); }
      });
