  ...
  factory: function (require) {
    ...
    function renderScan() {
      ...
      b.innerHTML =
        ...
        '<div class="lg-muted" style="margin-bottom:12px">直接调用系统 Windows Defender 引擎执行真实扫描。先选择查杀模式，再点"开始查杀"。快速查杀数分钟，全面查杀可能需要数小时，期间可关闭本窗口，扫描继续进行。</div>' +
        '<div class="lg-row">' +
        '<button class="' + quickCls + '" id="lg-scan-quick">' + I('zap', 12) + ' 快速查杀</button>' +
        '<button class="' + fullCls + '" id="lg-scan-full">' + I('search', 12) + ' 全面查杀</button>' +
        '<button class="lg-btn primary" id="lg-scan-start"' + (running ? ' disabled' : '') + '>' + I('play', 12) + ' 开始' + (pick === 'full' ? '全面' : '快速') + '查杀</button>' +
    ...
    function syncScanButtons() {
      ...
      s.innerHTML = I('play', 12) + ' 开始' + (pick === 'full' ? '全面' : '快速') + '查杀';
    }
