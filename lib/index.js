...
// 这是日常高频、零成本的清理动作（曾有一键释放内存/清待机内存功能，因需提权、
// UAC 等待长、收益有限，已按用户要求移除）。
// ---------------------------------------------------------------------------
...
async function route(req, res, url) {
  ...
  if (path === '/ping') {
    return json(res, 200, { ok: true, service: 'liang-guard', version: '1.2.0', admin: await isAdmin() });
  }
  if (path === '/config') {
    if (req.method === 'POST') {
...
// ---------------------------------------------------------------------------
// 挂载到 DSH webserver（同源）：/plugins/dsh-liang-guard/api/*
