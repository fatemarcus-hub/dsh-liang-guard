...
// 带宽测速：真实下载 + 真实上传（大小来自设置卡片；PS5.1 关掉进度条渲染，否则速度被严重拖慢）
// 下行走 speed.cloudflare.com/__down（Range 式），上行走 __up（POST 固定字节回显）。
// 各环节失败不拖垮整体：下行失败降级小文件，上行失败单独报 null。
function psSpeed(primaryBytes, fallbackBytes, upBytes) {
  return `
$ErrorActionPreference='Stop'
$ProgressPreference='SilentlyContinue'
[Net.ServicePointManager]::SecurityProtocol=[Net.SecurityProtocolType]::Tls12
function Test-Down($bytes){
...
function Test-Up($bytes){
  $data=[byte[]]::new($bytes)
  ([Random]::new()).NextBytes($data)
  $sw=[Diagnostics.Stopwatch]::StartNew()
  $r=Invoke-WebRequest -Uri 'https://speed.cloudflare.com/__up' -Method Post -Body $data -ContentType 'application/octet-stream' -UseBasicParsing -TimeoutSec 40
  $sw.Stop()
  [math]::Round($bytes*8/1KB/$sw.ElapsedMilliseconds,2)
}
$down=$null
try { $down=Test-Down ${primaryBytes} } catch { try { $down=Test-Down ${fallbackBytes} } catch { $down=$null } }
$up=$null
try { $up=Test-Up ${upBytes} } catch { $up=$null }
[pscustomobject]@{ mbps=$down; upMbps=$up } | ConvertTo-Json -Compress
`;
}
...
// 一键释放内存（真实机制）：
// 1) 清空系统待机内存列表（EmptyStandbyList，即卫士工具"内存加速"的真身，
//    实测可用内存可立即回升数百 MB）——需要 SeProfileSingleProcessPrivilege（仅管理员令牌）。
// 2) 尽力收缩可访问进程的工作集（自己进程/低完整性进程）。
// 关键坑：TOKEN_PRIVILEGES 结构体里 LUID 必须拆成 LowPart/HighPart 两个字段，
// 用 long 会让结构体膨胀到 20 字节（正确 16 字节），AdjustTokenPrivileges 静默失效
// → 特权"启用成功"却是假的 → EmptyWorkingSet/NtSetSystemInformation 全报拒绝访问。
const PS_MEMFREE = `
$ErrorActionPreference='Continue'
[Console]::OutputEncoding=[Text.Encoding]::UTF8
function AvailMB{
  try { [math]::Round((Get-Counter '\Memory\Available MBytes' -ErrorAction Stop).CounterSamples[0].CookedValue,0) }
  catch { [math]::Round((Get-CimInstance Win32_OperatingSystem).FreePhysicalMemory/1024,0) }
}
Add-Type -TypeDefinition @'
using System; using System.Diagnostics; using System.Runtime.InteropServices;
public class LgMemPriv {
  ...
  [DllImport("advapi32.dll", SetLastError=true)] public static extern bool AdjustTokenPrivileges(IntPtr t, bool d, ref TOKEN_PRIVILEGES p, uint l, IntPtr o, IntPtr r);
  public static int Enable(string name) {
    IntPtr t; if (!OpenProcessToken(Process.GetCurrentProcess().Handle, 0x28, out t)) return Marshal.GetLastWin32Error();
    LUID l; if (!LookupPrivilegeValue(null, name, out l)) return Marshal.GetLastWin32Error();
    TOKEN_PRIVILEGES p = new TOKEN_PRIVILEGES(); p.PrivilegeCount = 1; p.Privileges.Luid = l; p.Privileges.Attributes = 2;
    if (!AdjustTokenPrivileges(t, false, ref p, 0, IntPtr.Zero, IntPtr.Zero)) return Marshal.GetLastWin32Error();
    return 0;
  }
  [DllImport("ntdll.dll")] public static extern int NtSetSystemInformation(int cls, ref SMLC cmd, int len);
  [StructLayout(LayoutKind.Sequential)] public struct SMLC { public uint Command; }
  public static int EmptyStandbyList() { SMLC cmd = new SMLC(); cmd.Command = 4; return NtSetSystemInformation(80, ref cmd, 4); }
}
'@
$seProfileRet=[LgMemPriv]::Enable('SeProfileSingleProcessPrivilege')
$seDebugRet=[LgMemPriv]::Enable('SeDebugPrivilege')
$before=AvailMB
# 1) 尽力收缩可访问进程的工作集（大多数他人进程即使提权也会被拒——如实计数）
Add-Type -Namespace LgW -Name Psapi -MemberDefinition '[DllImport("psapi.dll")] public static extern int EmptyWorkingSet(IntPtr h);'
Add-Type -Namespace LgW -Name Kern -MemberDefinition '[DllImport("kernel32.dll")] public static extern IntPtr OpenProcess(uint access, bool inherit, int pid); [DllImport("kernel32.dll")] public static extern bool CloseHandle(IntPtr h);'
$targets=Get-Process | Where-Object { $_.WorkingSet64 -gt 100MB } | Sort-Object WorkingSet64 -Descending | Select-Object -First 30
$ok=0; $fail=0
foreach($p in $targets){
  $h=[LgW.Kern]::OpenProcess(0x0200 -bor 0x0400, $false, $p.Id)
  if($h -ne [IntPtr]::Zero){
    if([LgW.Psapi]::EmptyWorkingSet($h) -ne 0){ $ok++ } else { $fail++ }
    [void][LgW.Kern]::CloseHandle($h)
  } else { $fail++ }
}
# 2) 清空待机内存列表（主菜：可用内存立即可见回升）
$standbyOK=0
try { $standbyOK=[LgMemPriv]::EmptyStandbyList() } catch { $standbyOK=-1 }
Start-Sleep -Seconds 2
$after=AvailMB
[pscustomobject]@{ trimmed=$ok; skipped=$fail; freedMB=[math]::Round($after-$before,0); freeNowMB=$after; standbyOK=$standbyOK; seProfileRet=$seProfileRet; seDebugRet=$seDebugRet } | ConvertTo-Json -Compress
`;

// ---------------------------------------------------------------------------
// 一键清理缓存（无需管理员、秒级完成）：系统临时文件 + 浏览器缓存。
// 只动用户可写的已知缓存位置，绝不含用户文档；占用中的文件自动跳过。
// 与"释放内存"（提权+慢）彻底分离——这是日常高频、零成本的清理动作。
// ---------------------------------------------------------------------------
const PS_CACHE_CLEAN = `
$ErrorActionPreference='SilentlyContinue'
[Console]::OutputEncoding=[Text.Encoding]::UTF8
function DirMB($p){
...
$targets=@()
$targets += [pscustomobject]@{ name='系统临时文件'; path=$env:TEMP }
$chrome="$env:LOCALAPPDATA\Google\Chrome\User Data\Default\Cache"
$edge="$env:LOCALAPPDATA\Microsoft\Edge\User Data\Default\Cache"
$ff="$env:LOCALAPPDATA\Mozilla\Firefox\Profiles"
if(Test-Path $chrome){ $targets += [pscustomobject]@{ name='Chrome 缓存'; path=$chrome } }
if(Test-Path $edge){ $targets += [pscustomobject]@{ name='Edge 缓存'; path=$edge } }
if(Test-Path $ff){ $targets += [pscustomobject]@{ name='Firefox 缓存'; path=$ff } }
$rows=@()
$totalFreed=0.0
foreach($t in $targets){
  ...
  $after=DirMB $t.path
  $f=[math]::Round($before-$after,1)
  $totalFreed += $f
  $rows += [pscustomobject]@{ name=$t.name; beforeMB=$before; freedMB=$f }
}
[pscustomobject]@{ totalFreedMB=[math]::Round($totalFreed,1); rows=$rows } | ConvertTo-Json -Compress -Depth 3
`;

// ---------------------------------------------------------------------------
// 一键释放内存（提权助手）：普通权限 EmptyWorkingSet 必然全失败（Windows 硬限制），
// 参照腾讯/360 卫士的做法原理——它们安装的 SYSTEM 服务替它干，我们不做常驻服务，
// 改为点击时弹一次 UAC 授权，把同一段脚本交给提权后的 powershell 执行，
// 结果写回临时文件再由宿主读取。保持插件轻量，代价是每次释放需一次授权确认。
// ---------------------------------------------------------------------------
function runElevatedMemFree() {
  return new Promise((resolve) => {
    const ps1 = path.join(os.tmpdir(), 'liangguard-memfree.ps1');
    const out = path.join(os.tmpdir(), 'liangguard-memfree.json');
    try {
      fs.unlinkSync(out);
    } catch {}
    // 脚本主体复用 PS_MEMFREE，最后一行改为写文件（PS5.1 读无 BOM 的 UTF-8 时
    // 中文注释会乱码但仅注释，不影响执行；脚本体本身是 ASCII）
    let script = PS_MEMFREE.replace(
      /ConvertTo-Json -Compress\s*$/m,
      "ConvertTo-Json -Compress | Out-File -FilePath '" + out + "' -Encoding utf8"
    );
    fs.writeFileSync(ps1, script, 'utf8');
    // -Verb RunAs 触发 UAC。注意：PS 5.1 的 Start-Process -Verb RunAs -Wait 并不可靠，
    // 包装进程可能提前退出——因此不依赖 close 事件，改为轮询结果文件（最多 60s）。
    const child = spawn('powershell.exe', [
      '-NoProfile', '-NonInteractive', '-ExecutionPolicy', 'Bypass', '-Command',
      `Start-Process -FilePath 'powershell.exe' -Verb RunAs -Wait -ArgumentList '-NoProfile','-ExecutionPolicy','Bypass','-File','${ps1}'`,
    ], { windowsHide: true, stdio: ['ignore', 'ignore', 'ignore'] });
    let done = false;
    const started = Date.now();
    const finish = (val) => { if (!done) { done = true; clearTimeout(timer); try { child.kill('SIGKILL'); } catch {} resolve(val); } };
    const timer = setTimeout(() => finish({ uacCancelled: true, timeout: true }), 150000);
    const poll = () => {
      if (done) return;
      try {
        const r = JSON.parse(fs.readFileSync(out, 'utf8'));
        finish(r); // 结果文件就绪（无论 -Wait 是否提前返回）
        return;
      } catch {}
      // 用户点 UAC 需要时间：给足 120s（HTTP 侧与客户端均无更短超时）
      if (Date.now() - started > 120000) { finish({ uacCancelled: true }); return; }
      setTimeout(poll, 500);
    };
    child.on('error', () => finish(null));
    child.on('close', () => { /* 不在这里读文件，轮询接管 */ });
    poll();
  });
}
...
async function route(req, res, url) {
  ...
  if (path === '/mem/free' && req.method === 'POST') {
    ...
    // 非管理员先在本进程试一次（自己的进程/低完整性进程可能成功）
    const r = await runPsJson(PS_MEMFREE, 90000).catch(() => null);
    if (r && r.trimmed != null) {
      // 一个都没收缩成功 + 非管理员 → 升级为提权助手（弹一次 UAC 真刀真枪干）
      if (!admin && r.trimmed === 0 && r.skipped > 0) {
        const elevated = await runElevatedMemFree();
        if (elevated && !elevated.uacCancelled && elevated.trimmed != null) {
          return json(res, 200, { ok: true, admin: false, elevated: true, result: elevated });
        }
        if (elevated && elevated.uacCancelled) {
          return json(res, 200, { ok: false, error: '已取消授权——未收缩任何进程内存' });
        }
        return json(res, 200, { ok: false, error: '提权助手执行失败（请检查是否拒绝了 UAC 授权）' });
      }
      return json(res, 200, { ok: true, admin, result: r });
    }
    return json(res, 200, { ok: false, error: '内存释放执行失败' });
  }
  if (path === '/history') {
  ...
  if (path === '/net/speed' && req.method === 'POST') {
    ...
    const mb = cfg.speedMB || 10;
    const primary = mb * 1000000;
    const fallback = Math.max(2, Math.round(mb / 5)) * 1000000;
    const up = Math.max(2, Math.round(mb / 3)) * 1000000; // 上行小一些，避免慢速上行拖太久
    const s = await runPsJson(psSpeed(primary, fallback, up), 120000).catch(() => null);
    if (!s) return json(res, 200, { ok: false, error: '带宽测试失败（网络不通或被防火墙拦截）' });
    return json(res, 200, { ok: true, mbps: s.mbps, upMbps: s.upMbps });
  }
  if (path === '/cache/clean' && req.method === 'POST') {
    // 无需提权、秒级完成：只清用户可写的已知缓存位置（临时文件 + 浏览器缓存）
    const r = await runPsJson(PS_CACHE_CLEAN, 90000).catch(() => null);
    if (!r || r.totalFreedMB == null) return json(res, 200, { ok: false, error: '清理执行失败' });
    return json(res, 200, { ok: true, totalFreedMB: r.totalFreedMB, rows: r.rows || [] });
  }
