...
$ErrorActionPreference='Continue'
[Console]::OutputEncoding=[Text.Encoding]::UTF8
# 可用内存用 Get-Counter（含待机列表），收缩工作集释放的内存进待机区，
# 用 FreePhysicalMemory 测永远显示 0（那是"真空闲"，不含待机）
function AvailMB{
...
$after=AvailMB
[pscustomobject]@{ trimmed=$ok; skipped=$fail; freedMB=[math]::Round($after-$before,0); freeNowMB=$after } | ConvertTo-Json -Compress
`;
...
function runElevatedMemFree() {
  return new Promise((resolve) => {
    ...
    // -Verb RunAs 触发 UAC；用户取消时 Start-Process 抛错、退出码非 0
    ...
    let done = false;
    const timer = setTimeout(() => { if (!done) { try { child.kill('SIGKILL'); } catch {} } }, 90000);
    ...
    child.on('close', () => {
      ...
      try {
        ...
        // 没写结果文件 = 用户取消了 UAC 或提权失败
        resolve({ uacCancelled: true });
      }
    });
  });
}
