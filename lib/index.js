...
$topDirs=[array]($dirs.GetEnumerator() | Sort-Object Value -Descending | Select-Object -First 15 | ForEach-Object {
  [pscustomobject]@{ name=$_.Key; sizeGB=[math]::Round($_.Value/1GB,2) }
})
$topFiles=[array]($files | Sort-Object sizeMB -Descending | Select-Object -First 15)
...
function startDiskAnalyze(drive) {
  ...
  child.on('close', () => {
    ...
    if (m >= 0) {
      ...
      try {
        diskJob.result = JSON.parse(tail);
        // 防呆：单条结果会被 ConvertTo-Json 序列化成对象而非数组，客户端 map 会炸
        if (diskJob.result && !Array.isArray(diskJob.result.dirs)) diskJob.result.dirs = [];
        if (diskJob.result && !Array.isArray(diskJob.result.files)) diskJob.result.files = [];
      } catch { diskJob.result = null; }
...
async function route(req, res, url) {
  ...
  if (path === '/disk/cancel' && req.method === 'POST') {
    ...
    // 杀整棵进程树（与查杀取消同法），保证递归遍历进程彻底终止
    try {
      spawn('taskkill', ['/PID', String(diskJob.child.pid), '/T', '/F'], { windowsHide: true, stdio: 'ignore' });
    } catch { try { diskJob.child.kill('SIGKILL'); } catch {} }
