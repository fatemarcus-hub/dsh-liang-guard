...
// 上下文会无限挂起（实测：引擎 0 CPU 干等），MpCmdRun 流式输出、退出码明确：
// 0=发现并处置威胁，2=干净，其他=真实错误。
...
& $mp -Scan -ScanType ${scanType}
$code=$LASTEXITCODE
if($code -ne 0 -and $code -ne 2){ Write-Output ('SCAN-ERROR: MpCmdRun exit code ' + $code); Exit 0 }
...
// ---------------------------------------------------------------------------
let scanJob = null; // { id, mode, startedAt, child, running, error, threats, finishedAt }
...
function startScan(mode) {
  ...
  child.stdout.on('data', (c) => {
    buf += c;
    if (/SCAN-ERROR: /.test(buf)) scanJob.error = buf.match(/SCAN-ERROR: (.*)/)[1].trim();
  });
  ...
  child.on('close', () => {
    ...
    if (m >= 0) {
      ...
      scanJob.error = '扫描进程异常退出（Defender 服务端可能仍在后台扫描，稍后刷新查看威胁记录）';
    }
    pushHistory('scans', {
...
async function route(req, res, url) {
  ...
  if (path === '/ping') {
    return json(res, 200, { ok: true, service: 'liang-guard', version: '1.1.0', admin: await isAdmin() });
  }
  if (path === '/config') {
    if (req.method === 'POST') {
  ...
  if (path === '/scan/cancel' && req.method === 'POST') {
    ...
    try { scanJob.child.kill('SIGKILL'); } catch {}
    scanJob.running = false; scanJob.finishedAt = Date.now(); scanJob.error = '已取消（注意：Defender 服务端的扫描可能继续后台执行）';
